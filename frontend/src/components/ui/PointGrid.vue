    ...
    <v-container>
        ...
        <div class="panel">
            <div class="gs-bundle-of-buttons" style="max-height:10vh;">
                ...
                <v-btn style="margin-left: 5px;" @click="deductPointDialog = true" class="contrast-primary-text" small color="primary" :disabled="!hasRole('User')">
                    <v-icon small>mdi-minus-circle-outline</v-icon>포인트 사용
                </v-btn>
                <v-dialog v-model="deductPointDialog" width="500">
                    <DeductPoint
                        @closeDialog="deductPointDialog = false"
...
export default {
    ...
    data: () => ({
        path: 'points',
        deductPointDialog: false,
    }),
    watch: {
    },
    methods:{
        async deductPoint(params){
            try{
                var path = "deductPoint".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, params)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','DeductPoint 성공적으로 처리되었습니다.')
                for(var i = 0; i< this.value.length; i++){
                    if(this.value[i] == this.selectedRow){
                        this.value[i] = temp.data
                    }
                }
                this.deductPointDialog = false
            }catch(e){
                console.log(e)
            }
        },
