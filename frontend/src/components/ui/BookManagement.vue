    ...
    <v-container fluid>
        ...
        <v-card>
            ...
            <v-card-text>
                ...
                <div class="d-flex flex-wrap justify-end align-center" style="gap: 10px;">
                    <v-text-field v-model="userId" label="사용자 ID" dense hide-details solo class="me-2"
                        style="max-width: 150px;" placeholder="ex: 1"></v-text-field>
                    ...
                    <v-col v-for="book in books" :key="book.bookId" cols="12" sm="6" md="4" lg="3">
                        <v-card class="book-card" flat outlined>
                            <v-img
                                ...
                                height="300px" cover class="align-end">
                            ...
                            <v-card-actions>
                                <v-spacer></v-spacer>
                                <v-btn icon small title="수정/작업"
                                    @click="openEditDialog(book)"><v-icon>mdi-pencil</v-icon></v-btn>
                                <v-btn icon small title="상세보기"
                                    @click="getBookDetails(book.bookId)"><v-icon>mdi-eye</v-icon></v-btn>
                                <v-btn icon small title="삭제" @click="deleteBook(book.bookId)"><v-icon
        ...
        <v-dialog v-model="editDialog" max-width="2500px" persistent>
            <v-card>
                <v-card-title>
                    <span class="text-h5">{{ editedItem.bookId ? '도서 작업' : '글 작성' }}</span>
                </v-card-title>
                <v-card-text>
                ...
                <v-card-actions>
                    ...
                    <v-btn v-if="editedItem.bookId" color="teal" dark @click="handleCommand('updatemetadata')">메타데이터
                        업데이트</v-btn>
                    <v-btn v-if="editedItem.bookId" color="purple" dark @click="handleCommand('submitbookcommand')">출간
                        요청</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
        ...
        <v-dialog v-model="detailsDialog" max-width="600px">
            <v-card>
                ...
                <v-card-text v-if="detailedItem">
                    ...
                    <p><strong>가격:</strong> {{ detailedItem.price ? `${detailedItem.price.toLocaleString()}원` : '-' }}
                    </p>
