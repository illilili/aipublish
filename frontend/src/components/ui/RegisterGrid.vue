    ...
    <div class="right-pane">
      <v-sheet class="signup-form-sheet" max-width="400">
        ...
        <v-form @submit.prevent="handleSignUp">
          ...
          <v-checkbox 
            v-model="form.isKtCustomer" 
            label="KT 고객이신가요? (혜택 제공)" 
            density="compact"
            class="mt-2"
          ></v-checkbox>
          ...
          <v-btn :loading="loading" :disabled="!isFormValid" type="submit" color="primary" size="x-large" block class="mt-6">
            가입하기
          </v-btn>
        </v-form>
...
export default {
  ...
  data() {
    return {
      // ✅ [수정] form 객체에 isKtCustomer 속성 추가
      form: { name: '', email: '', password: '', passwordConfirm: '', isKtCustomer: false },
      agreements: { all: false, terms: false, privacy: false },
      isPasswordVisible: false, isPasswordConfirmVisible: false, loading: false,
      rules: {
  ...
  watch: {
    'agreements.all'(newValue) {
      this.agreements.terms = newValue; this.agreements.privacy = newValue;
    },
  ...
  computed: {
    isFormValid() {
      return (
        this.form.name && /.+@.+\..+/.test(this.form.email) &&
        this.form.password && this.form.password === this.form.passwordConfirm &&
        this.agreements.terms && this.agreements.privacy
      );
    },
  },
 methods: {
    async handleSignUp() {
      ...
      const payload = {
        ...
        isKtCustomer: this.form.isKtCustomer,
        subscription: false,
        isAdmin: false,
      };
      try {
...
.left-pane .brand-content { position: relative; z-index: 1; text-align: center; animation: fadeIn 1.5s ease-in-out; }
.right-pane { flex: 1; display: flex; justify-content: center; align-items: center; background-color: #ffffff; overflow-y: auto; padding: 24px 0; }
.signup-form-sheet { width: 100%; padding: 24px; max-width: 400px; }
.agreements { padding: 8px 12px; background-color: #f9f9f9; border: 1px solid #eee; border-radius: 8px; }
@media (max-width: 960px) { .left-pane { display: none; } }
